...
typedef ItemConNormId = ({VentaResponse item, String? normId});

/// Resuelve el [Perfume] de un ítem de orden vía su id normalizado.
/// Devuelve null si el perfume no está en [perfumesMap] (ej. catálogo
/// desactualizado o perfume eliminado).
Perfume? perfumeDeItem(ItemConNormId entry, Map<String, Perfume> perfumesMap) =>
    entry.normId != null ? perfumesMap[entry.normId] : null;

/// Etiqueta a mostrar cuando el perfume del ítem no está en el catálogo.
String nombreFallbackItem(VentaResponse item) =>
...
class OrdenAgrupada {
  OrdenAgrupada({required this.idCompra, required this.items})
      : itemsConNormId = items
            .map((i) => (
                  item: i,
                  normId:
                      i.idPerfume != null ? normalizeId(i.idPerfume!) : null,
                ))
            .toList();
  ...
  final List<ItemConNormId> itemsConNormId;
